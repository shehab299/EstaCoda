...
write_wrapper() {
  ...
  # Remove existing wrapper if it's ours
  if [ -L "$wrapper" ] || [ -e "$wrapper" ]; then
    if [ -L "$wrapper" ]; then
      ...
      if echo "$target" | grep -qi "estacoda"; then
        ...
        echo "Warning: refusing to overwrite existing non-EstaCoda symlink: $wrapper" >&2
        echo "Binary installed at: $INSTALL_DIR/estacoda" >&2
        return 0
      fi
    elif grep -Eq "Generated by scripts/(install|setup-estacoda)\\.sh|EstaCoda" "$wrapper" 2>/dev/null; then
      rm -f "$wrapper"
    else
